...
diagnostic_CLI_args="--trainer.logger=pytorch_lightning.loggers.TensorBoardLogger --trainer.logger.name=$SLURM_JOB_NAME --trainer.logger.version=$SLURM_JOB_ID" #--trainer.logger.save_dir=ChemtabUQ_TBlogs"
...
srun --ntasks-per-node=2 python ChemtabUQ.py fit --data.class_path=MeanRegressorDataModule $lightning_CLI_args --trainer.logger.save_dir=CT_logs_Mu
mkdir mean_regressor
mv model.* mean_regressor

if ! [ $MEAN_ONLY ]; then
	srun --ntasks-per-node=2 python ChemtabUQ.py fit --data.class_path=UQRegressorDataModule --data.mean_regressor_fn=mean_regressor/model.ckpt $lightning_CLI_args --trainer.logger.save_dir=CT_logs_Sigma 
	mkdir UQ_regressor
